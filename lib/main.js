...
function hoursInMonth( month, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1QFAZ,LU', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1QFAX,LV', month ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1QF7f,LW', year ) );
		}
		mon = month;
		yr = year;
	}
	if ( isInteger( mon ) && (mon < 1 || mon > 12) ) {
		throw new RangeError( format( '1QFAa,LX', mon ) );
	}
	mon = lowercase( mon.toString() );
	hrs = HOURS_IN_MONTH[ mon ];
	if ( hrs === void 0 ) {
		throw new Error( format( '1QFAb,LY', mon ) );
	}
